...
"""GUI representative: foreign debris token + extrude F9 (ownership-free).

Plan: .agents/doc/f9_extrude_plan_2026-08-15.md v3.1 §4-2 row 8.
Recipe: paint HISTORY_TOKEN → ed.undo_push("debris") → extrude (mirrored)
→ ed.undo_redo. undo_post must see debris tokens and no vid layer.
Three-domain match ⇒ continue (remirror).

Run::

    cmd.exe /c "tmp\\run_menu_reg.bat 42 test_extrude_f9_foreign.py"
"""
...
OBJECT_NAME = "YSE_F9Foreign"
MESH_NAME = "YSE_F9ForeignMesh"
MARKER_OK = "YSE_EXTRUDE_F9_FOREIGN_OK"
MARKER_FAILED = "YSE_EXTRUDE_F9_FOREIGN_FAILED"
...
def fail(message=""):
    if message:
        print(f"YSE_EXTRUDE_F9_FOREIGN_ERROR={message}", flush=True)
    traceback.print_exc()
    print(f"YSE_EXTRUDE_F9_FOREIGN_PHASE={STATE.get('phase')}", flush=True)
...
def build_mesh():
    ...
    with override():
        ...
        bpy.ops.ed.undo_push(message="YSE F9 foreign baseline")
    return obj
...
def snapshot_undo_post_layers():
    ...
    vid_layer = bm.verts.layers.int.get(layer_names.VERT_SESSION_ID_LAYER)
    tokens = set()
    if token_layer is not None:
        tokens = {int(face[token_layer]) for face in bm.faces if int(face[token_layer])}
    STATE["undo_post_has_token"] = token_layer is not None and bool(tokens)
    STATE["undo_post_has_vid"] = vid_layer is not None
    STATE["undo_post_tokens"] = tokens
    print(
        f"YSE_EXTRUDE_F9_FOREIGN_UNDO_POST token={STATE['undo_post_has_token']} "
        f"vid={STATE['undo_post_has_vid']} tokens={sorted(tokens)}",
        flush=True,
    )
...
def instrument_f9():
    ...
    def logged():
        ...
        print(
            f"YSE_EXTRUDE_F9_FOREIGN_DISCRIMINATOR result={result} sessions={STATE['f9_sessions_after_prepare']}",
            flush=True,
        )
        return result
...
def after_extrude():
    try:
        ...
        bm = bmesh.from_edit_mesh(obj.data)
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_FOREIGN_NET_AFTER_EXTRUDE={got}", flush=True)
...
def do_f9():
    try:
        ...
        print(f"YSE_EXTRUDE_F9_FOREIGN_UNDO_REDO={result}", flush=True)
...
def after_f9():
    try:
        ...
        assert STATE.get("undo_post_has_token") is True, "undo_post must see debris HISTORY_TOKEN"
        assert STATE.get("undo_post_has_vid") is False, "undo_post must not see vid layer"
        assert DEBRIS_TOKEN in STATE.get("undo_post_tokens", set()), STATE.get("undo_post_tokens")
        obj = test_object()
        bm = bmesh.from_edit_mesh(obj.data)
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_FOREIGN_NET_AFTER_F9={got}", flush=True)
        ...
        assert prepare is True, f"foreign debris must continue (result={prepare})"
        STATE["finished"] = True
...
def start():
    try:
        ...
        obj = test_object()
        bm = bmesh.from_edit_mesh(obj.data)
        paint_debris_token(bm)
        bmesh.update_edit_mesh(obj.data, loop_triangles=False, destructive=False)
        with override():
            push = bpy.ops.ed.undo_push(message="debris")
        assert push == {"FINISHED"}, push
        bm = bmesh.from_edit_mesh(test_object().data)
        select_single_face(bm)
        bmesh.update_edit_mesh(test_object().data, loop_triangles=False, destructive=False)
        with override():
            push = bpy.ops.ed.undo_push(message="YSE F9 foreign prepared")
        assert push == {"FINISHED"}, push
        STATE["baseline"] = topology_counts(bmesh.from_edit_mesh(test_object().data))
        print(
            f"YSE_EXTRUDE_F9_FOREIGN_BASELINE={STATE['baseline']} drag_threshold={click_drag_threshold_px()}",
            flush=True,
        )
