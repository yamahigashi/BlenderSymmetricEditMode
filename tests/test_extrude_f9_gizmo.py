...
"""GUI representative: GIZMO_ADOPTED EXTRUDE_CONTEXT, then one ed.undo_redo.

Plan: .agents/doc/f9_extrude_plan_2026-08-15.md v3.1 §4-2 row 7.
Invocation follows tests/test_extrude_gizmo.py double_extrude_undo
(programmatic INVOKE_DEFAULT under an armed gizmo poll).

Run::

    cmd.exe /c "tmp\\run_menu_reg.bat 42 test_extrude_f9_gizmo.py"
"""
...
from ydd_symmetric_edit import gizmo_adopt, keymaps, layer_names, operators  # noqa: E402  # noqa: E402
from ydd_symmetric_edit import history as history_module  # noqa: E402

OBJECT_NAME = "YSE_F9Gizmo"
MESH_NAME = "YSE_F9GizmoMesh"
MARKER_OK = "YSE_EXTRUDE_F9_GIZMO_OK"
MARKER_FAILED = "YSE_EXTRUDE_F9_GIZMO_FAILED"
...
def fail(message=""):
    if message:
        print(f"YSE_EXTRUDE_F9_GIZMO_ERROR={message}", flush=True)
    traceback.print_exc()
    print(f"YSE_EXTRUDE_F9_GIZMO_PHASE={STATE.get('phase')}", flush=True)
...
def build_mesh():
    ...
    with override():
        ...
        bpy.ops.ed.undo_push(message="YSE F9 gizmo baseline")
    return obj
...
def set_intercept(active):
    for keymap, item in keymaps._REGISTERED_ITEMS:
        if keymap.name == TOOL_KEYMAP_NAME and item.idname == keymaps.INTERCEPT_OPERATOR:
            item.active = active
...
def instrument_f9():
    ...
    def logged():
        ...
        print(
            f"YSE_EXTRUDE_F9_GIZMO_DISCRIMINATOR result={result} sessions={STATE['f9_sessions_after_prepare']}",
            flush=True,
        )
        return result
...
def after_extrude():
    try:
        ...
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_GIZMO_NET_AFTER_EXTRUDE={got}", flush=True)
        assert got == EXPECTED_NET, f"net {got} != {EXPECTED_NET}"
        assert_x_symmetric(bm)
        assert_layers_removed(bm)
        record = latest_record()
        assert record is not None, "missing COMMITTED gizmo record"
        assert record.status == "COMMITTED", record
        assert record.session.prepare_disposition == "APPLY", record.session
        assert record.session.route == gizmo_adopt.GIZMO_ROUTE, record.session.route
        STATE["old_token"] = record.session.history_token
        STATE["old_sequence"] = record.sequence
        print(
            f"YSE_EXTRUDE_F9_GIZMO_OLD_TOKEN={STATE['old_token']} seq={STATE['old_sequence']} "
            f"route={record.session.route}",
            flush=True,
        )
...
def do_f9():
    try:
        ...
        print(f"YSE_EXTRUDE_F9_GIZMO_UNDO_REDO={result}", flush=True)
...
def after_f9():
    try:
        ...
        bm = bmesh.from_edit_mesh(obj.data)
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_GIZMO_NET_AFTER_F9={got}", flush=True)
        ...
        print(f"YSE_EXTRUDE_F9_GIZMO_NEW_TOKEN={new_token}", flush=True)
...
def invoke_gizmo():
    try:
        ...
        dx, dy = 0, max(80, click_drag_threshold_px() + 16)
        with override():
...
def start():
    try:
        ...
        with override():
            push = bpy.ops.ed.undo_push(message="YSE F9 gizmo prepared")
        assert push == {"FINISHED"}, push
        STATE["baseline"] = topology_counts(bmesh.from_edit_mesh(obj.data))
        print(
            f"YSE_EXTRUDE_F9_GIZMO_BASELINE={STATE['baseline']} drag_threshold={click_drag_threshold_px()}",
            flush=True,
        )
        with override():
        ...
        set_intercept(False)
        keymaps._sync_gizmo_poll()
        bpy.app.timers.register(invoke_gizmo, first_interval=0.4)
    except BaseException:
        fail()
    return None
