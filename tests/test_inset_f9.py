...
"""Dedicated one-shot Inset F9 regression (contract v5.2 §5C / §7 / §9).

One ``ed.undo_redo`` per Blender process: confirm a one-sided inset through
the intercept route, adjust ``thickness`` on the native active operator, run
F9 once, and assert the undo_post re-intervention replayed the mirror side.

Run with Blender's real window/event loop::

    blender --factory-startup --enable-event-simulate --python test_inset_f9.py
"""
...
MARKER_OK = "YSE_INSET_F9_TEST_OK"
MARKER_FAILED = "YSE_INSET_F9_TEST_FAILED"
FACE_PLUS = (0.75, 0.25)
PRECISION = 5
...
def fail(message=""):
    if message:
        print(f"YSE_INSET_F9_ERROR={message}", flush=True)
    traceback.print_exc()
    print(f"YSE_INSET_F9_REPORTS={list(inset_bevel._REPORTS)}", flush=True)
    print(f"YSE_INSET_F9_TOKEN={inset_bevel._ACTIVE_TOKEN}", flush=True)
...
def build_grid():
    ...
    for face in bm.faces:
        ...
        if abs(center.x - FACE_PLUS[0]) < 1e-4 and abs(center.y - FACE_PLUS[1]) < 1e-4:
            target = face
            break
    if target is None:
        raise AssertionError("fixture face not found")
    target.select = True
...
def route_ready():
    ...
    for keymap in addon_config.keymaps:
        for item in keymap.keymap_items:
            if item.idname == keymaps.INSET_BEVEL_INTERCEPT_OPERATOR and item.type == "I" and item.active:
                return True
    return False
...
def main_gen():
    ...
    region = STATE["region"]
    obj = build_grid()
    yield from wait_until(route_ready, 10.0, "inset intercept route never appeared")
    cx = region.x + region.width // 2
    cy = region.y + region.height // 2
    window.event_simulate(type="MOUSEMOVE", value="NOTHING", x=cx, y=cy)
    yield 0.1
    window.event_simulate(type="I", value="PRESS", x=cx, y=cy)
    window.event_simulate(type="I", value="RELEASE", x=cx, y=cy)
    yield 0.3
    ...
    assert active is not None and "inset" in active.bl_idname.lower(), (
        f"active_operator is not the native inset: {getattr(active, 'bl_idname', None)}"
    )
    active.properties.thickness = 0.25
    ...
    obj = bpy.context.active_object
    assert topology_counts(obj) == confirmed_counts, "F9 changed the inset topology"
    after_f9 = vertex_multiset(obj)
    assert after_f9 != before_f9, "adjusted thickness produced identical geometry"
    ...
    ), "select_mirrored OFF: F9 left mirror-side selection"
