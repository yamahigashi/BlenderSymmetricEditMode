...
"""GUI representative: EXTRUDE_NORMAL via KMI E, then one ed.undo_redo.

Plan: .agents/doc/f9_extrude_plan_2026-08-15.md v3.1 §4-2 row 1.

Run::

    cmd.exe /c "tmp\\run_menu_reg.bat 42 test_extrude_f9_kind_normal.py"
"""
...
OBJECT_NAME = "YSE_F9KindNormal"
MESH_NAME = "YSE_F9KindNormalMesh"
MARKER_OK = "YSE_EXTRUDE_F9_KIND_NORMAL_OK"
MARKER_FAILED = "YSE_EXTRUDE_F9_KIND_NORMAL_FAILED"
...
def fail(message=""):
    if message:
        print(f"YSE_EXTRUDE_F9_KIND_NORMAL_ERROR={message}", flush=True)
    traceback.print_exc()
    print(f"YSE_EXTRUDE_F9_KIND_NORMAL_PHASE={STATE.get('phase')}", flush=True)
...
def build_mesh():
    ...
    with override():
        ...
        bpy.ops.ed.undo_push(message="YSE F9 kind normal baseline")
    return obj
...
def instrument_f9():
    ...
    def logged():
        ...
        pointer = int(operator.as_pointer()) if operator is not None else None
        obj = test_object()
        bm = bmesh.from_edit_mesh(obj.data)
        print(
            f"YSE_EXTRUDE_F9_KIND_NORMAL_UNDO_POST "
            f"active={getattr(operator, 'bl_idname', None)} ptr={pointer} "
            f"counts={topology_counts(bm)} "
            f"sel_v={sum(1 for vertex in bm.verts if vertex.select)} "
            f"sel_e={sum(1 for edge in bm.edges if edge.select)} "
            f"sel_f={sum(1 for face in bm.faces if face.select)} "
            f"mode={tuple(bpy.context.tool_settings.mesh_select_mode)}",
            flush=True,
        )
        for record in operators._HISTORY_RECORDS.values():
            print(
                f"YSE_EXTRUDE_F9_KIND_NORMAL_RECORD kind={record.session.tool_kind} "
                f"status={record.status} ptr={record.session.native_operator_pointer} "
                f"token={record.session.history_token} obj={record.session.object_name}",
                flush=True,
            )
        result = original()
        ...
        print(
            f"YSE_EXTRUDE_F9_KIND_NORMAL_DISCRIMINATOR result={result} sessions={STATE['f9_sessions_after_prepare']}",
            flush=True,
        )
        return result
...
def try_mutate_f9_property():
    operator = bpy.context.active_operator
    if operator is None:
        print("YSE_EXTRUDE_F9_KIND_NORMAL_ACTIVE_OP=None", flush=True)
        return False
    print(f"YSE_EXTRUDE_F9_KIND_NORMAL_ACTIVE_OP={operator.bl_idname}", flush=True)
    ...
    for child in children:
        ...
        try:
            if isinstance(old, (int, float)) and not isinstance(old, bool):
                ...
                props.value = new
                print(f"YSE_EXTRUDE_F9_KIND_NORMAL_MUTATE scalar {old} -> {props.value}", flush=True)
                mutated = True
            else:
                vec = list(old)
                if len(vec) == 3:
                    ...
                    print(f"YSE_EXTRUDE_F9_KIND_NORMAL_MUTATE vector {tuple(vec)} -> {tuple(props.value)}", flush=True)
                    mutated = True
        except Exception as exc:
            print(f"YSE_EXTRUDE_F9_KIND_NORMAL_MUTATE_FAIL={exc!r}", flush=True)
    return mutated
...
def after_extrude():
    try:
        ...
        bm = bmesh.from_edit_mesh(obj.data)
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_KIND_NORMAL_NET_AFTER_EXTRUDE={got}", flush=True)
        ...
        print(
            f"YSE_EXTRUDE_F9_KIND_NORMAL_OLD_TOKEN={STATE['old_token']} seq={STATE['old_sequence']}",
            flush=True,
        )
...
def do_f9():
    try:
        ...
        STATE["property_mutated"] = mutated
        print(f"YSE_EXTRUDE_F9_KIND_NORMAL_PROPERTY_MUTATED={int(mutated)}", flush=True)
        ...
        print(f"YSE_EXTRUDE_F9_KIND_NORMAL_UNDO_REDO={result}", flush=True)
...
def after_f9():
    try:
        ...
        bm = bmesh.from_edit_mesh(obj.data)
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_KIND_NORMAL_NET_AFTER_F9={got}", flush=True)
        ...
        print(f"YSE_EXTRUDE_F9_KIND_NORMAL_NEW_TOKEN={new_token}", flush=True)
...
def start():
    try:
        ...
        with override():
            push = bpy.ops.ed.undo_push(message="YSE F9 kind normal prepared")
        assert push == {"FINISHED"}, push
        STATE["baseline"] = topology_counts(bmesh.from_edit_mesh(obj.data))
        print(
            f"YSE_EXTRUDE_F9_KIND_NORMAL_BASELINE={STATE['baseline']} drag_threshold={click_drag_threshold_px()}",
            flush=True,
        )
