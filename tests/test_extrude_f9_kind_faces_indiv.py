...
"""GUI representative: EXTRUDE_FACES_INDIV via tool drag, then one ed.undo_redo.

Plan: .agents/doc/f9_extrude_plan_2026-08-15.md v3.1 §4-2 row 4.

Run::

    cmd.exe /c "tmp\\run_menu_reg.bat 42 test_extrude_f9_kind_faces_indiv.py"
"""
...
OBJECT_NAME = "YSE_F9KindFacesIndiv"
MESH_NAME = "YSE_F9KindFacesIndivMesh"
MARKER_OK = "YSE_EXTRUDE_F9_KIND_FACES_INDIV_OK"
MARKER_FAILED = "YSE_EXTRUDE_F9_KIND_FACES_INDIV_FAILED"
...
def fail(message=""):
    if message:
        print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_ERROR={message}", flush=True)
    traceback.print_exc()
    print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_PHASE={STATE.get('phase')}", flush=True)
...
def build_mesh():
    ...
    with override():
        ...
        bpy.ops.ed.undo_push(message="YSE F9 kind faces_indiv baseline")
    return obj
    ...
    def poll():
        try:
            ...
            if time.monotonic() - started > 8.0:
                raise RuntimeError("Extrude Individual LEFTMOUSE CLICK_DRAG intercept was not registered")
            return 0.05
        except BaseException:
            fail()
        return None
...
def instrument_f9():
    ...
    def logged():
        ...
        print(
            f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_DISCRIMINATOR result={result} "
...
def try_mutate_f9_property():
    ...
    if operator is None:
        print("YSE_EXTRUDE_F9_KIND_FACES_INDIV_ACTIVE_OP=None", flush=True)
        return False
    print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_ACTIVE_OP={operator.bl_idname}", flush=True)
    ...
    for child in children:
        ...
        try:
            if isinstance(old, (int, float)) and not isinstance(old, bool):
                ...
                props.value = new
                print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_MUTATE scalar {old} -> {props.value}", flush=True)
                mutated = True
            else:
                vec = list(old)
                if len(vec) == 3:
                    ...
                    print(
                        f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_MUTATE vector {tuple(vec)} -> {tuple(props.value)}",
                        flush=True,
                    )
                    mutated = True
        except Exception as exc:
            print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_MUTATE_FAIL={exc!r}", flush=True)
    return mutated
...
def after_extrude():
    try:
        ...
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_NET_AFTER_EXTRUDE={got}", flush=True)
        ...
        print(
            f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_OLD_TOKEN={STATE['old_token']} seq={STATE['old_sequence']}",
            flush=True,
        )
...
def do_f9():
    try:
        ...
        print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_PROPERTY_MUTATED={int(mutated)}", flush=True)
        ...
        print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_UNDO_REDO={result}", flush=True)
...
def after_f9():
    try:
        ...
        bm = bmesh.from_edit_mesh(obj.data)
        got = live_net(bm)
        print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_NET_AFTER_F9={got}", flush=True)
        ...
        print(f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_NEW_TOKEN={new_token}", flush=True)
...
def start():
    try:
        ...
        with override():
            push = bpy.ops.ed.undo_push(message="YSE F9 kind faces_indiv prepared")
        assert push == {"FINISHED"}, push
        STATE["baseline"] = topology_counts(bmesh.from_edit_mesh(obj.data))
        print(
            f"YSE_EXTRUDE_F9_KIND_FACES_INDIV_BASELINE={STATE['baseline']} "
